...
## Latest Enhancement: Auto-Save, Date Validation & Publish Toggle (Implemented 2025-07-02)

### Overview
The Activity Editor now features automatic saving, eliminating the need for manual save actions. Changes are saved automatically in the background every 2 seconds after modifications or immediately when switching tabs.

Additionally, the editor now enforces IATI-compliant date field rules based on activity status.
...
2. **Added Auto-Save Status Indicator**:
   - Shows "Saving..." with spinning icon during save
   - Shows "Saved [time]" with checkmark after successful save
   - Shows "Unsaved changes" with warning icon when changes are pending

3. **Required Fields Protection**:
   - Auto-save only requires Activity Title to be completed
   - Removed sector allocation requirement for publishing
   - Modal dialog appears if attempting to auto-save without Activity Title

4. **Navigation Protection**:
   - Browser warns users before leaving page with unsaved changes
   - Auto-save triggers before tab changes

5. **IATI-Compliant Date Field Validation**:
   - Dynamic enable/disable of date fields based on activity status:
     - Pipeline/Identification: Only planned dates available
     - Implementation: Planned dates + actual start (no actual end)
     - Completion and beyond: All dates available
   - Visual indicators show why fields are disabled
   - Fields automatically update when status changes

6. **Publish Toggle Switch**:
   - Replaced publish button with toggle switch in header
   - Shows "Unpublished" / "Published" status clearly
   - Larger toggle (125% scale) with bold labels
   - Instant publishing without scrolling to footer
   - Green highlight when published
   - Only visible to users with publish permissions
   - Removed save status indicators for cleaner UI

### User Benefits
- No data loss from forgetting to save
- Seamless editing experience
- Clear visibility of save status
