...
interface TopDistrictsChartProps {
  data: RankedItem[];
  grandTotal: number;
}
...
export function TopDistrictsChart({
  ...
  const handleExport = () => {
    ...
    exportChartToCSV(exportData, `top-districts`);
          ...
          {data.map((district) => (
            <TableRow key={district.id}>
              ...
              <TableCell className="text-right font-mono">
                {formatCurrencyFull(district.value)}
              </TableCell>
              <TableCell className="text-right">
                {grandTotal > 0 ? ((district.value / grandTotal) * 100).toFixed(1) : 0}%
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
