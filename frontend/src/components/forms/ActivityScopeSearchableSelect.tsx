...
const allOptions: Option[] = IATI_ACTIVITY_SCOPE.flatMap(group =>
...
interface ActivityScopeSearchableSelectProps {
  value?: string;
...
export function ActivityScopeSearchableSelect({
  value,
  onValueChange,
  placeholder = "Select activity scope...",
  disabled = false,
  className,
  dropdownId = "activity-scope-select",
  side,
  align = "start",
}: ActivityScopeSearchableSelectProps) {
        ...
        >
          <Command>
            <div className="flex items-center border-b px-3 py-2">
              ...
              <input
                placeholder="Search activity scope..."
                value={searchQuery}
            ...
            <CommandList>
              {Object.entries(groupedOptions).map(([groupName, options]) => (
                <CommandGroup key={groupName} className="p-0">
                  <div className="px-3 py-1.5 text-helper font-semibold text-muted-foreground bg-muted/50">
                    {groupName}
                  </div>
                  {options.map((option) => (
                    <CommandItem
              ...
              {Object.keys(groupedOptions).length === 0 && (
                <div className="py-8 text-center">
                  <div className="text-body text-muted-foreground">
                    No activity scope options found.
                  </div>
...
export type { ActivityScopeSearchableSelectProps };