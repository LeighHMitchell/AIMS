...
import { ChevronsUpDown, Check, Search, AlertTriangle } from "lucide-react";
...
interface BudgetIdentifierVocabularySelectProps {
  value?: string;
...
export function BudgetIdentifierVocabularySelect({
  value,
  onValueChange,
  placeholder = "Select budget identifier vocabulary...",
  disabled = false,
  className,
  dropdownId = "budget-vocabulary-select",
  side,
  align = "start",
}: BudgetIdentifierVocabularySelectProps) {
  const { isOpen, setOpen } = useDropdownState(dropdownId);
  const [searchQuery, setSearchQuery] = React.useState("");

  const selectedOption = BUDGET_IDENTIFIER_VOCABULARIES.find(option => option.code === value);

  const filteredOptions = React.useMemo(() => {
    if (!searchQuery) return BUDGET_IDENTIFIER_VOCABULARIES;
    
    const query = searchQuery.toLowerCase();
    return BUDGET_IDENTIFIER_VOCABULARIES.filter(option => 
  ...
  return (
    <div className={cn("pb-6", className)}>
      <Popover open={isOpen} onOpenChange={setOpen}>
        <PopoverTrigger
          className={cn(
            "flex h-10 w-full items-center justify-between rounded-md border border-input bg-white px-3 py-2 text-body ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 hover:bg-muted transition-colors",
              ...
              <CommandGroup>
                {filteredOptions.map((option) => (
                  <CommandItem
                    key={option.code}
                    onSelect={() => {
                      // Don't allow selection of withdrawn vocabularies
                      if (!option.withdrawn) {
                    ...
                    className={cn(
                      ...
                      option.withdrawn && "opacity-50 cursor-not-allowed hover:bg-transparent"
                    )}
                    disabled={option.withdrawn}
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn(
                          "font-medium",
                          option.withdrawn ? "text-muted-foreground" : "text-foreground"
                        )}>
                          {option.name}
                        </span>
                        {option.withdrawn && (
                      ...
                      {option.withdrawn && (
                        <div className="text-helper text-amber-700 mt-1.5 italic">
                          This vocabulary has been withdrawn from the IATI standard
                        </div>
                      )}
                    </div>
                  </CommandItem>
...
export type { BudgetIdentifierVocabularySelectProps };
