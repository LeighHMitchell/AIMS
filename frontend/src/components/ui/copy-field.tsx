...
export function CopyField({
  ...
  if (variant === "inline") {
    return (
      <div className={cn("flex items-center gap-3", className)}>
        {!hideLabel && (
          <label className={cn("text-body font-medium text-foreground min-w-0 flex-shrink-0", labelClassName)}>
            {label}:
          </label>
        )}
        <div className={cn(
          "flex items-center justify-between bg-muted border border-border rounded-md px-3 py-2 text-body flex-1 min-w-0",
  ...
  return (
    <div className={cn("space-y-2", className)}>
      {!hideLabel && (
        <label className={cn("text-body font-medium text-foreground", labelClassName)}>
          {label}
        </label>
      )}
      <div className={cn(
        "flex items-center justify-between bg-muted border border-border rounded-md px-3 py-2 text-body",
        "focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500",
        fieldClassName
      )}>
        <span className={cn(
...
export function CopyFieldGroup({
  ...
  return (
    <div className={cn("space-y-4", className)}>
      {title && (
        <h3 className="text-body font-semibold text-foreground border-b border-border pb-2">
          {title}
        </h3>
      )}
      <div className={cn(
