...
export function PaginationPrevious({ onClick, "aria-disabled": ariaDisabled, className = "" }: { onClick?: () => void; "aria-disabled"?: boolean; className?: string }) {
  return (
    <button
      ...
      className={`px-2 py-1 rounded text-sm text-muted-foreground hover:bg-muted disabled:text-gray-300 disabled:cursor-not-allowed ${className}`}
...
export function PaginationLink({ isActive, onClick, children, className = "" }: { isActive?: boolean; onClick?: () => void; children: React.ReactNode; className?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`px-2 py-1 rounded text-sm ${isActive ? "bg-muted text-foreground font-semibold" : "text-muted-foreground hover:bg-muted"} ${className}`}
