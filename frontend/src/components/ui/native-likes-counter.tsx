...
export function NativeLikesCounter({
  ...
  const getVariantStyles = () => {
    ...
    switch (variant) {
      case "subtle":
        return cn(base, "bg-accent/50 hover:bg-accent")
      case "outline":
        return cn(
          base,
      ...
      case "ghost":
        return cn(base, "bg-transparent hover:bg-accent/50")
      default:
        return cn(
          base,
  ...
  const getLikedBackground = () => {
    if (!isLiked) return undefined
    return { backgroundColor: colors.backgroundLiked }
  }
        ...
        <motion.div className="relative flex items-center justify-center">
          <motion.div animate={isLiked ? { scale: [1, 1.15, 1] } : { scale: 1 }} transition={{ duration: 0.2 }}>
            <Heart
              ...
              style={isLiked ? { fill: colors.heartActive, color: colors.heartActive } : undefined}
            />
          </motion.div>
        </motion.div>
