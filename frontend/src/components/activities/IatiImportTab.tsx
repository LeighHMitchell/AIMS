                  ...
                  {activity.recipientCountries.map((country: string, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {countryName && (
                          <span className="text-slate-900 text-xs">{countryName}</span>
                        )}
                      </div>
                    );
                  })}
...
<iati-activities version="2.03">
  ...
  const parseXmlFile = async () => {
    ...
    if (!currentActivityData.id && activityId) {
      console.log('[IATI Import Debug] Fetching activity data before parsing');
      try {
        const data = await fetchBasicActivityWithCache(activityId);
        setCurrentActivityData({
          id: data.id,
    ...
    try {
      ...
      if (importMethod === 'file' && selectedFile) {
        ...
        // Fetch from URL
        console.log('[IATI Import Debug] Fetching from URL');
        setImportStatus({ stage: 'uploading', progress: 30 });
      ...
      console.log('[IATI Import Debug] Current activity data recipient_countries:', currentActivityData.recipient_countries);
      ...
      if (shouldShowField('location') && parsedActivity.locations && parsedActivity.locations.length > 0) {
        parsedActivity.locations.forEach((location: any, locIndex: number) => {
          ...
          fields.push({
            ...
            console.log('[IatiImportTab] Debug - locIndex:', locIndex, 'currentActivityData.locations:', currentActivityData.locations);
              ...
              if (orgData.ref) {
                ...
                if (searchResponse.ok) {
                  ...
                  if (matchedOrg) {
                    ...
                    console.log(`[IATI Import] ⚠️ Found exact IATI ref match:`, {
                      ref: orgData.ref,
                    ...
                    console.log(`[IATI Import] ✓ No organization found with exact IATI ref "${orgData.ref}"`);
