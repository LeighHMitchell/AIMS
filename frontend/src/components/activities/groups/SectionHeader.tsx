...
export const SECTION_HELP_TEXTS: Record<string, string> = {
  ...
  "program-logic": "Build a design-level program logic (results framework / theory of change) for this investment, above the machine-readable IATI results. Pick a framework preset (DFAT, USAID, World Bank, EU, OECD DAC, GAC) or define custom tiers, then add statements of change and connect them as a many-to-many contribution graph. Optionally link nodes to existing IATI indicators as measurement evidence, build activity sub-logics that roll up into the investment, and lock a baseline with named revisions. The logic graph is never exported to IATI; only the linked indicators are.",
  ...
  "fund-overview": "A summary dashboard showing the fund's balance (total contributions minus total disbursements), key metrics such as number of development partners and child activities, top contributors, top sectors, and a quarterly disbursement trend.",
  "fund-contributions": "View all contributions into this fund by development partner organisation. Shows pledged, committed, and received amounts with year-by-year breakdowns. Data is drawn from incoming transactions and participating organisations with a funding role.",
