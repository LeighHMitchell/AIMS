...
export default function TransactionModal({
  ...
  }, [formData.currency, formData.value_date, formData.transaction_date]);

  // Calculated USD value
  const calculatedUsdValue = formData.value && exchangeRate
    ? Math.round(formData.value * exchangeRate * 100) / 100
    : null;

  // Auto-fetch exchange rate when currency or date changes (only if not manual)
  useEffect(() => {
    if (!exchangeRateManual && formData.currency) {
  ...
  }, [formData.currency, formData.value_date, formData.transaction_date, exchangeRateManual, fetchExchangeRate]);

  // Reset exchange rate state when modal opens
  useEffect(() => {
    if (open && transaction) {
      const existingManual = (transaction as any).exchange_rate_manual ?? false;
      const existingRate = (transaction as any).exchange_rate_used ?? null;
      setExchangeRateManual(existingManual);
  ...
  const getTransactionPayload = (formData: Partial<Transaction>) => {
    ...
    const allowed = [
      ...
      'is_humanitarian',
      'exchange_rate_used', 'value_usd', 'exchange_rate_manual'
    ];
    const payload: any = {};
  ...
  const handleSubmit = async (statusOverride?: string) => {
    ...
    // Add exchange rate fields to submission
    if (exchangeRate !== null) {
      submissionData.exchange_rate_used = exchangeRate;
      submissionData.exchange_rate_manual = exchangeRateManual;
      submissionData.value_usd = calculatedUsdValue;
    }
  ...
  const LabelSaveIndicator = ({ children, isSaving, isSaved }: { children: React.ReactNode; isSaving?: boolean; isSaved?: boolean }) => (
    <Label className="text-body font-medium flex items-center gap-2">
      {children}
      {isSaving && <span className="text-helper text-blue-500">(saving...)</span>}
            ...
            {(transaction as any)?.auto_ref && (
              <div className="inline-flex items-center gap-2 text-muted-foreground whitespace-nowrap">
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText((transaction as any).auto_ref);
                    toast.success(`Copied ${(transaction as any).auto_ref}`);
                  }}
                  title="Click to copy"
                ...
                <HelpTextTooltip side="bottom" align="end" size="sm">
                  A unique, immutable identifier (TXN-####) automatically generated by the system when this transaction was created. It cannot be edited.
                </HelpTextTooltip>
              </div>
            )}
          </div>
        ...
        <ScrollArea className="flex-1 overflow-y-auto">
          <div className="px-8 py-6 space-y-8">
                          ...
                          {TRANSACTION_TYPE_OPTIONS.filter((opt: { code: string; name: string; desc: string }) => {
                            ...
                            <React.Fragment key={opt.code}>
                              <CommandItem
                                ...
                                className={cn("flex flex-col items-start px-4 py-3 gap-1 hover:bg-accent/50 focus:bg-accent data-[selected]:bg-accent transition-colors", formData.transaction_type === opt.code && "bg-accent")}
                              >
                    ...
                    <div className="flex items-center justify-between h-12 w-full rounded-md border border-input bg-background px-4 py-3 hover:bg-accent/50 transition-colors">
                      <div className="flex items-center space-x-3">
                        <Checkbox
                          ...
                          className="h-5 w-5"
                        />
                        <Label htmlFor="validated" className="text-body font-medium cursor-pointer flex items-center gap-1">
                          Mark as Validated
                  ...
                  <div className="flex items-center justify-between min-h-[24px]">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Label
                        ...
                        className="text-body font-normal cursor-pointer text-muted-foreground"
                      >
            ...
            {formData.currency && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between min-h-[24px]">
                    <LabelWithInfoAndSave
                      helpText="The exchange rate used to convert the transaction value to USD. Automatically fetched from historical rates based on the value date. Toggle the switch to enter a manual rate instead."
                      isSaving={false}
                      isSaved={false}
                      hasValue={exchangeRate != null}
                    >
                      Exchange Rate
                    </LabelWithInfoAndSave>
                    {formData.currency !== 'USD' && (
                      <div className="flex items-center gap-2">
                        <Label htmlFor="exchange_rate_mode" className={cn("text-body font-normal cursor-pointer", exchangeRateManual ? "text-orange-500 font-medium" : "text-muted-foreground")}>
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setExchangeRate(isNaN(value) ? null : value);
                      }}
                      disabled={!exchangeRateManual || isLoadingRate || formData.currency === 'USD'}
                      className={cn(
                        "w-full",
                        (!exchangeRateManual || formData.currency === 'USD') && "bg-muted cursor-not-allowed"
                      )}
                    ...
                    {!isLoadingRate && !exchangeRateManual && formData.currency !== 'USD' && (
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
                  ...
                  {rateInfoMessage && !rateError && (
                    <p className="text-helper text-muted-foreground">{rateInfoMessage}</p>
                  )}
                </div>
                <div className="space-y-2">
                  <div className="flex items-center min-h-[24px]">
                    <LabelWithInfoAndSave
                      helpText="The transaction value converted to US Dollars using the exchange rate shown. This is calculated automatically from the original value and exchange rate."
                      isSaving={false}
                      isSaved={false}
                      hasValue={calculatedUsdValue != null}
                    >
                      USD Value
                    </LabelWithInfoAndSave>
                  </div>
            ...
            <div className="space-y-4">
                      ...
                      >
                        <span className="flex-1 min-w-0 truncate">
                          {formData.disbursement_channel ? (
                            ...
                            <span className="text-muted-foreground text-body">Select disbursement channel</span>
                          )}
                        </span>
                        <ChevronsUpDown className="h-4 w-4 shrink-0 opacity-50 ml-2 mt-0.5" />
                      </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-[var(--radix-popover-trigger-width)] min-w-[320px] max-w-xl p-0 shadow-lg border" align="start" sideOffset={4}>
                    <Command>
                          ...
                          .map(([code, option], idx, arr) => (
                            <React.Fragment key={code}>
                              <CommandItem
                                ...
                                className={cn("flex flex-col items-start px-4 py-3 gap-1 hover:bg-accent/50 focus:bg-accent data-[selected]:bg-accent transition-colors", formData.disbursement_channel === code && "bg-accent")}
                              >
                                <div className="flex items-start gap-3 w-full">
                                  <span className="text-xs font-mono text-muted-foreground bg-muted px-2 py-0.5 rounded min-w-[28px] text-center flex-shrink-0">
                                    {code}
                                  </span>
                                ...
                                <div className="text-helper text-muted-foreground pl-10 leading-relaxed w-full">
                                  {option.desc}
                                </div>
                              </CommandItem>
                              {idx < arr.length - 1 && (
              ...
              {/* Row 2: Flow Type and Finance Type */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <LabelWithInfoAndSave
              ...
              {/* Row 3: Tied Status and Humanitarian */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <LabelWithInfoAndSave
                ...
                <div className="space-y-2">
                  ...
                  <div className={cn(
                    ...
                    <span className={cn(
                      "text-body flex items-center gap-1.5",
                  ...
                  >
                    <div className="flex items-center justify-between w-full">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-body font-medium text-foreground">Geographic Targeting</span>
                        ...
                        <div className="flex items-center gap-6">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              disabled={isSubmitting}
                            />
                            <span className="text-body text-muted-foreground">Activity-level default</span>
                          </label>
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              type="radio"
                ...
                <CollapsibleTrigger asChild>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-body font-medium text-foreground">Multiple Aid Types</span>
                      {multipleAidTypesCount > 0 && (
                        <Badge variant="secondary" className="text-helper">{multipleAidTypesCount}</Badge>
                      )}
                    </div>
                  </button>
                </CollapsibleTrigger>
              ...
              <CollapsibleTrigger asChild>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-body font-medium text-foreground">System Identifiers</span>
                  </div>
                </button>
              </CollapsibleTrigger>
              <CollapsibleContent>
