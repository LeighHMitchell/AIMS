      ...
      <MainLayout>
        <div className="min-h-screen"><div className="w-full p-6">
          <Card><CardContent className="p-8 text-center">
            ...
            <h2 className="text-xl font-semibold text-slate-900 mb-2">Error Loading Sector</h2>
      ...
      <div className="min-h-screen">
        <div className="w-full p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <Button variant="ghost" size="sm" onClick={() => router.push('/sectors')} className="text-slate-600 hover:text-slate-900 hover:bg-slate-100">
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h1 className="text-2xl font-bold text-slate-900 mb-1">{sector.name}</h1>
                  <div className="flex items-center gap-2 flex-wrap mt-2">
                    <Badge variant="outline" className="border-slate-300 text-slate-700">{getLevelLabel(sector.level)}</Badge>
                    <Badge variant="outline" className="border-slate-300 text-slate-700">DAC CRS</Badge>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <Card className="border-slate-200">
              <CardHeader className="py-2 px-3"><CardTitle className="text-xs font-medium text-slate-600">Sub-sector Coverage</CardTitle></CardHeader>
              <CardContent className="px-1 pb-2">
            ...
            <TabsContent value="overview" className="space-y-6">
              <Card><CardHeader><CardTitle className="text-sm">Summary</CardTitle></CardHeader><CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div><p className="text-xs font-medium text-slate-600 mb-1">Aligned Activities</p><p className="text-3xl font-bold text-slate-900">{formatNumber(metrics.totalActivities)}</p><p className="text-xs text-slate-500 mt-0.5">in this sector</p></div>
              ...
              {subSectorBreakdown.length > 0 && (
                <Card><CardHeader><CardTitle className="text-sm">Sub-sector Coverage</CardTitle></CardHeader><CardContent>
                  ...
                  <p className="text-xs text-slate-400 mt-2">{subSectorBreakdown.filter(s => s.totalValue > 0).length} of {subSectorBreakdown.length} sub-sectors funded</p>
                </CardContent></Card>
              )}
            ...
            <TabsContent value="sub-sectors" className="space-y-6">
              <Card><CardHeader><CardTitle className="text-sm">{sector.level === 'sector' ? 'Related Sectors in Same Category' : 'Sub-Sector Breakdown'}</CardTitle></CardHeader><CardContent>
                {subSectorBreakdown.length > 0 ? (
                  ...
                  <p className="text-slate-500 text-sm text-center py-8">No sub-sector data available</p>
                )}
              </CardContent></Card>
            </TabsContent>
              ...
              {activityView === 'card' ? (
                ...
                <Card><CardContent className="p-0"><div className="overflow-x-auto">
                  <table className="w-full text-xs"><thead><tr className="border-b border-slate-200 bg-slate-50">
                    ...
                    <th className="text-center py-2.5 px-3 text-slate-600 font-medium">Alloc %</th>
                    ...
                    {paginatedActivities.map(activity => (
                      <tr key={activity.id} className="border-b border-slate-100 hover:bg-slate-50">
                        ...
                        <td className="py-2 px-3 text-center text-slate-500">{activity.sectorPercentage}%</td>
