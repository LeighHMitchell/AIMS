...
// GET - Retrieve bookmarked organization IDs for the current user
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[Organization Bookmarks API] Error fetching bookmarks:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[Organization Bookmarks API] Error adding bookmark:', error);
    ...
    console.error('[Organization Bookmarks API] Error in POST:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
