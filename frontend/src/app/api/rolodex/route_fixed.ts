...
export async function GET(request: NextRequest) {
  try {
    ...
    // Apply user filters
    if (filters.search) {
      usersQuery = usersQuery.or(`name.ilike.%${filters.search}%,email.ilike.%${filters.search}%,first_name.ilike.%${filters.search}%,last_name.ilike.%${filters.search}%`);
    }
    if (filters.role) {
    ...
    let contactsQuery = supabase
      ...
      .select(`
        ...
        created_at,
        updated_at,
        activities(title, title_narrative, recipient_country),
        organizations(name, country)
      `)
    ...
    if (users) {
      users.forEach((user: any) => {
        transformedPeople.push({
          ...
          activity_title: null,
          country_code: user.organizations?.country || null,
    ...
    if (contacts) {
      contacts.forEach((contact: any) => {
        ...
        transformedPeople.push({
          ...
          organization_name: contact.organizations?.name || contact.organisation || null,
          activity_title: contact.activities?.title_narrative || contact.activities?.title || null,
          country_code: contact.organizations?.country || contact.activities?.recipient_country || null,
