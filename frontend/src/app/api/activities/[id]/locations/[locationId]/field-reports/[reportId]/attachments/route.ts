...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  if (!(await assertReportInScope(supabase, reportId, locationId, activityId))) {
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  if (error) {
    console.error('[FieldReportAttachments API] Create error:', error);
    return NextResponse.json({ error: `Failed to create attachment: ${error.message}` }, { status: 500 });
  }
