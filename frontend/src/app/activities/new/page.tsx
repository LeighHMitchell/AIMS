    ...
    <div className="bg-card rounded-lg shadow-sm border border-border p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6" data-tour="editor-identifier">
        ...
        <div className="space-y-2">
          <LabelSaveIndicator
            isSaving={false}
            isSaved={!!general.autoRef}
            hasValue={!!general.autoRef}
            className="text-foreground"
          >
            <div className="flex items-center gap-2">
              System Generated ID
              <HelpTextTooltip>
                A unique, immutable identifier (ACT-####) automatically generated by the system when the activity is created. It cannot be edited.
              </HelpTextTooltip>
            </div>
          </LabelSaveIndicator>
          <div className="relative">
            ...
            {general.autoRef && (
              <button
                onClick={() => handleCopy(general.autoRef, 'System Generated ID')}
                className="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 hover:bg-muted rounded"
                title="Copy System Generated ID"
                type="button"
              >
                <Copy className="w-4 h-4 text-muted-foreground hover:text-foreground" />
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(general.customDates || []).map((customDate: any, index: number) => (
                <div key={index} className="space-y-2">
                  ...
                  <div className="relative">
                    <CalendarIcon className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 flex-shrink-0 pointer-events-none ${customDate.date ? '' : 'text-muted-foreground'}`} />
                    <Input
                      type="text"
      ...
      try {
        ...
        if (activityId) {
          ...
          setGeneral({
            ...
            autoRef: data.autoRef || data.auto_ref || "",
