/**
 * IATI Standard CRS Channel Code Codelist (v2.03)
 * https://iatistandard.org/en/iati-standard/203/codelists/crschannelcode/
 * 
 * These codes classify the channel through which development assistance is delivered.
 */

export interface IATICRSChannelCode {
  code: string;
  name: string;
  description?: string;
  category?: string;
...
export const IATI_CRS_CHANNEL_CODES: IATICRSChannelCode[] = [
  ...
  // Non-Governmental Organisation (NGO) and Civil Society (20000)
  {
    code: "20000",
  ...
  // Public-Private Partnerships (PPP) and Networks (30000)
  {
    code: "30000",
  ...
  {
    code: "41310",
    name: "United Nations Department of Peace Operations - UN peacekeeping operations",
    category: "41300"
  },
  {
    code: "41312",
  ...
  {
    code: "41600",
    name: "Existing UN channels not included in Standard I",
    category: "41000"
  },
  {
    code: "42000",
  ...
  {
    code: "43002",
    name: "International Monetary Fund - Poverty Reduction and Growth - Heavily Indebted Poor Countries Debt Relief Initiative Trust Fund",
    category: "43000"
  },
  {
    code: "43003",
  ...
  {
    code: "43007",
    name: "IMF's Resilience and Sustainability Trust",
    category: "43000"
  },
  {
    code: "44000",
  ...
  {
    code: "47095",
    name: "Educational Quality and Assessment Programme",
    category: "47000"
  },
  {
    code: "47096",
  ...
  // University, college or other teaching institution, research institute or think-tank (51000)
  {
    code: "51000",
...
/**
 * Get CRS channel code name by code
 */
export function getCRSChannelCodeName(code: string): string {
  const channelCode = IATI_CRS_CHANNEL_CODES.find(c => c.code === code);
  return channelCode?.name || 'Unknown';
}

/**
 * Get CRS channel code description by code
 */
export function getCRSChannelCodeDescription(code: string): string {
  const channelCode = IATI_CRS_CHANNEL_CODES.find(c => c.code === code);
  return channelCode?.description || '';
}

/**
 * Get CRS channel codes grouped by category
 */
export function getCRSChannelCodesByCategory(): Record<string, IATICRSChannelCode[]> {
  const grouped: Record<string, IATICRSChannelCode[]> = {};
  
  IATI_CRS_CHANNEL_CODES.forEach(code => {
...
/**
 * Get CRS channel codes as options for select components
 */
export function getCRSChannelCodeOptions() {
  return IATI_CRS_CHANNEL_CODES
    .filter(code => !code.withdrawn)
    .map(code => ({
      code: code.code,
