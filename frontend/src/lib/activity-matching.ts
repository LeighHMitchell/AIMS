...
// Find similar activities using fuzzy matching
export function findSimilarActivities(
  ...
  for (const existingActivity of existingActivities) {
    ...
    if (newActivity.title && existingActivity.title) {
      const titleScore = stringSimilarity(newActivity.title, existingActivity.title);
      if (titleScore > 0.7) {
    ...
    if (newActivity.description && existingActivity.description) {
      const descScore = stringSimilarity(newActivity.description, existingActivity.description);
      if (descScore > 0.6) {
    ...
    if (dateRangesOverlap(
      ...
      existingActivity.plannedStartDate,
      existingActivity.plannedEndDate
    )) {
    ...
    if (locationsOverlap(newActivity.locations, existingActivity.locations)) {
