 ...
 * recycle-bin filtering), which columns to display, and which of those columns
 * count as a "data gap" (so a row is only shown when at least one is missing).
 *
 * The API route (app/api/data-clinic/entity/[entity]/route.ts) uses this to
 * query + flatten rows; the generic client component renders whatever columns
 * the route returns. Keep this module server-only friendly (no JSX).
 */

export type ClinicColumnType = 'text' | 'code' | 'date' | 'money' | 'percent'
...
export const CLINIC_ENTITIES: Record<string, ClinicEntityConfig> = {
  'planned-disbursements': {
    ...
    columns: [
      { key: 'type', label: 'Type', type: 'code', gap: true },
      ...
      { key: 'provider', label: 'Provider', type: 'text', gap: true },
      { key: 'receiver', label: 'Receiver', type: 'text', gap: true },
  ...
  sectors: {
    ...
    mapRow: (r) => ({
      ...
      sector_vocabulary: r.sector_vocabulary,
    }),
  },
  ...
  locations: {
    ...
    columns: [
      ...
      { key: 'latitude', label: 'Latitude', type: 'text', gap: true },
      { key: 'longitude', label: 'Longitude', type: 'text', gap: true },
    ...
    mapRow: (r) => ({
      ...
      location_reach: r.location_reach,
...
/** Public column shape (no functions) returned to the client. */
