-- Fix calculate_transparency_scores(): the original definition joined a
-- `partners` table via `activities.partner_id`, but neither the table nor the
-- column exist in this schema, so every call failed with
-- `relation "partners" does not exist` (breaking the Transparency Index Export
-- report and the transparency dashboard). Drop the dead join and return NULL
-- for partner_name (callers already COALESCE it to a fallback). Everything else
-- is unchanged. SECURITY DEFINER + search_path are preserved.
