-- Migration: Fix pivot_report_data view
-- Fixes:
-- 1. Tied status showing code instead of name (added CASE statement)
-- 2. Aid type, finance type, flow type showing codes (added CASE statements)
-- 3. Fiscal year type casting for consistent string handling
-- 4. Added code columns for all categorical fields (hidden by default in UI)
...
SELECT 
    ...
    -- Tied Status with human-readable names (FIXED: was showing code)
...
COMMENT ON VIEW pivot_report_data IS 'Denormalized view for pivot table reports. Joins activities, transactions, sectors, and organizations. Only includes published activities. Added human-readable names for tied_status, aid_type, finance_type, and flow_type.';
