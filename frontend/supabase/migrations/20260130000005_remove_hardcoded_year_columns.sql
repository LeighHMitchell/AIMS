-- Migration: Remove hardcoded fiscal year columns from pivot_report_data
-- 
-- This migration removes the static fiscal year columns (calendar_year, us_fiscal_year, etc.)
-- from the database view. Fiscal years are now calculated dynamically in the API layer
-- based on the active year types in the custom_years table.
--
-- This allows new fiscal year types to be added without requiring SQL migrations.
-- The API uses the effective_date column to calculate fiscal years for each year type.

-- Drop and recreate the view without hardcoded fiscal year columns
...
SELECT 
    ...
    -- NOTE: Fiscal year columns (calendar_year, us_fiscal_year, australian_fiscal_year, etc.)
    -- are now calculated dynamically in the API layer from custom_years table.
...
FROM (
    ...
    SELECT 
        ...
        'Planned Disbursement' AS transaction_type,
    ...
    SELECT 
        ...
        NULL::NUMERIC AS transaction_value_usd,
        NULL::NUMERIC AS planned_disbursement_value_usd,
        COALESCE(ab.usd_value, ab.value) AS budget_value_usd,
...
NOTE: Fiscal year columns are now calculated dynamically in the API layer from custom_years table.
Use effective_date for fiscal year calculations.
