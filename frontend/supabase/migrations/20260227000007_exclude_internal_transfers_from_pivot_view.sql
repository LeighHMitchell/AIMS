-- Migration: Exclude internal transfers from pivot_report_data view
--
-- When a pooled fund disburses to child activities, the same money appears
-- as both the fund's outgoing disbursement AND each child's incoming receipt.
-- This causes double-counting in portfolio-level analytics.
--
-- This migration adds WHERE clauses to the transaction portion of the
-- pivot_report_data view to exclude internal transfers:
-- - Outgoing transactions (types 2,3,4) where receiver_activity_uuid IS NOT NULL
-- - Incoming transactions (types 1,11,13) where provider_activity_uuid IS NOT NULL
--
-- Fund-level views (fund overview, contributions, disbursements tabs) are NOT
-- affected - they query the transactions table directly.
