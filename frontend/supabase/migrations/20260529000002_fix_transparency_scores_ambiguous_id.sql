-- Fix calculate_transparency_scores(): after the previous migration removed the
-- dead `partners` join, the function still failed at runtime with
-- `column reference "id" is ambiguous`. The RETURNS TABLE output columns
-- (id, title, reporting_org_id, reporting_org_name, partner_name, updated_at)
-- share names with the columns of the score_components CTE, so an unqualified
-- reference in the final SELECT/ORDER BY could not be resolved between the
-- OUT parameter and the CTE column.
--
-- Fix: alias the CTE as `sc` and qualify every CTE-sourced column reference in
-- the final SELECT. Logic, scoring, weights and the JSON breakdown are
-- unchanged. SECURITY DEFINER + search_path are preserved.
...
BEGIN
  ...
  SELECT
    ...
    sc.title,
    sc.reporting_org_id,
    COALESCE(sc.reporting_org_name, 'Unknown Organization') as reporting_org_name,
    COALESCE(sc.partner_name, 'Unknown Partner') as partner_name,
    sc.updated_at,
    -- Calculate Total Weighted Score
    ROUND(
      (
        sc.score_op_reg + sc.score_op_strat +
        sc.score_fin_budget + sc.score_fin_currency + sc.score_fin_disburse + sc.score_fin_capex +
        sc.score_attr_title + sc.score_attr_desc + sc.score_attr_dates + sc.score_attr_status +
        sc.score_join_loc + sc.score_join_sector +
        sc.score_perf_obj + sc.score_perf_results
      ) * sc.time_multiplier
    , 1) as total_score,
    ...
    jsonb_build_object(
      'operational_planning', jsonb_build_object(
        'score', (sc.score_op_reg + sc.score_op_strat),
        'details', jsonb_build_array(
          jsonb_build_object('label', 'MoU / Registration', 'points', sc.score_op_reg, 'max', 5),
          jsonb_build_object('label', 'Country Strategy', 'points', sc.score_op_strat, 'max', 5)
        )
      ),
      'finance', jsonb_build_object(
        'score', (sc.score_fin_budget + sc.score_fin_currency + sc.score_fin_disburse + sc.score_fin_capex),
        'details', jsonb_build_array(
          jsonb_build_object('label', 'Forward Budget (N+1)', 'points', sc.score_fin_budget, 'max', 10),
          jsonb_build_object('label', 'Currency Defined', 'points', sc.score_fin_currency, 'max', 5),
          jsonb_build_object('label', 'Disbursements (Current Year)', 'points', sc.score_fin_disburse, 'max', 10),
          jsonb_build_object('label', 'Capital Expenditure', 'points', sc.score_fin_capex, 'max', 5)
        )
      ),
      'attributes', jsonb_build_object(
        'score', (sc.score_attr_title + sc.score_attr_desc + sc.score_attr_dates + sc.score_attr_status),
        'details', jsonb_build_array(
          jsonb_build_object('label', 'Title Quality (>10 chars)', 'points', sc.score_attr_title, 'max', 5),
          jsonb_build_object('label', 'Description Quality (>80 chars)', 'points', sc.score_attr_desc, 'max', 10),
          jsonb_build_object('label', 'Valid Dates', 'points', sc.score_attr_dates, 'max', 5),
          jsonb_build_object('label', 'Valid Status', 'points', sc.score_attr_status, 'max', 5)
        )
      ),
      'joining_up', jsonb_build_object(
        'score', (sc.score_join_loc + sc.score_join_sector),
        'details', jsonb_build_array(
          jsonb_build_object('label', 'Locations Mapped', 'points', sc.score_join_loc, 'max', 10),
          jsonb_build_object('label', 'Sector Assigned', 'points', sc.score_join_sector, 'max', 5)
        )
      ),
      'performance', jsonb_build_object(
        'score', (sc.score_perf_obj + sc.score_perf_results),
        'details', jsonb_build_array(
          jsonb_build_object('label', 'Objectives/Indicators', 'points', sc.score_perf_obj, 'max', 10),
          jsonb_build_object('label', 'Results Reported', 'points', sc.score_perf_results, 'max', 10)
        )
      ),
      'multiplier', sc.time_multiplier
    ) as breakdown
