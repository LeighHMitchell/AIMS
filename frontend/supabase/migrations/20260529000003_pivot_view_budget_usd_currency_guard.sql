-- Harden pivot_report_data so non-USD budgets never surface their raw foreign
-- amount in the USD column.
--
-- The live view (migration 20260227000007) computed:
--     COALESCE(ab.value, 0) AS budget_value_usd
-- i.e. it used the RAW budget value regardless of currency, completely ignoring
-- the already-converted usd_value column. For an AUD budget this emitted the raw
-- AUD figure (e.g. 21,261,161) in the "Budget Amount (USD)" / amount_usd /
-- weighted_amount_usd columns, inflating pivot & report totals.
--
-- Fix: prefer the converted usd_value; fall back to the raw value ONLY when the
-- budget is already in USD; for an un-converted non-USD budget emit NULL
-- (amount_usd / weighted_amount_usd already COALESCE NULL -> 0), so raw foreign
-- currency can never masquerade as USD. Only the budget_value_usd expression in
-- the Budget branch changes; the rest of the view is reproduced verbatim from
-- 20260227000007 (the current live definition).
...
SELECT
    ...
    -- Weighted amount that applies sector_percentage
    ...
    -- Flag indicating if the amount is in original currency (not converted to USD)
...
FROM (
    ...
    WHERE a.publication_status = 'published'
      ...
      -- Exclude internal transfers to avoid double-counting pooled fund flows:
      -- Outgoing transactions (2=Commitment, 3=Disbursement, 4=Expenditure) where
      -- receiver_activity_uuid is set means money is going to another activity in the system
      AND NOT (t.transaction_type::TEXT IN ('2', '3', '4') AND t.receiver_activity_uuid IS NOT NULL)
      -- Incoming transactions (1=Incoming Funds, 11=Incoming Commitment, 13=Incoming Pledge)
      -- where provider_activity_uuid is set means money came from another activity in the system
      AND NOT (t.transaction_type::TEXT IN ('1', '11', '13') AND t.provider_activity_uuid IS NOT NULL)

    UNION ALL

    -- =========================================
    -- PLANNED DISBURSEMENTS (unchanged)
    -- =========================================
    SELECT
        a.id AS activity_id,
        a.iati_identifier,
    ...
    SELECT
        ...
        NULL::NUMERIC AS transaction_value_usd,
        NULL::NUMERIC AS planned_disbursement_value_usd,
        CASE WHEN ab.usd_value IS NOT NULL THEN ab.usd_value WHEN ab.currency = 'USD' THEN ab.value ELSE NULL END AS budget_value_usd,  -- USD if converted; raw value only when already USD; else NULL (never expose raw foreign currency as USD)
...
4. INTERNAL TRANSFERS: Transactions between activities in the system (pooled fund
   flows) are excluded to prevent double-counting. This is controlled by:
   - receiver_activity_uuid (outgoing types 2/3/4)
   - provider_activity_uuid (incoming types 1/11/13)
