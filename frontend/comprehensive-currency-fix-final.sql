-- Comprehensive Currency Conversion Fix Script (Final Corrected Version)
...
-- Step 4: Reset failed transactions for retry (with proper counting)
DO $$
DECLARE
    reset_count INTEGER;
BEGIN
    -- Reset failed transactions and count them
...
VALUES 
    ...
    ('EUR', 'USD', 1.04, CURRENT_DATE - INTERVAL '180 days', 'fallback'),
    ('EUR', 'USD', 1.07, CURRENT_DATE - INTERVAL '365 days', 'fallback'),
    ...
    ('GBP', 'USD', 1.25, CURRENT_DATE - INTERVAL '7 days', 'fallback'),
    ('GBP', 'USD', 1.29, CURRENT_DATE - INTERVAL '30 days', 'fallback'),
    ('GBP', 'USD', 1.24, CURRENT_DATE - INTERVAL '90 days', 'fallback'),
    ('GBP', 'USD', 1.35, CURRENT_DATE - INTERVAL '180 days', 'fallback'),
    ('GBP', 'USD', 1.30, CURRENT_DATE - INTERVAL '365 days', 'fallback')
ON CONFLICT (from_currency, to_currency, rate_date) DO NOTHING;

-- Step 8: Manual conversion function for immediate use (FIXED - no variable conflicts)
...
BEGIN
    -- Process pending transactions (using table alias to avoid conflicts)
    FOR txn_record IN 
        SELECT t.uuid, t.currency, t.value, t.transaction_date, t.value_date
        FROM transactions t
        WHERE t.currency != 'USD' 
        AND t.value_usd IS NULL 
        AND t.usd_convertible != false
        AND t.value > 0
    ...
    LOOP
        ...
        -- Look for exchange rate (find the most recent rate before or on the conversion date)
        ...
        IF rate_value IS NOT NULL THEN
            ...
            txn_currency := txn_record.currency;
        ...
        -- Reset rate_value for next iteration
        rate_value := NULL;
    END LOOP;
END;
$$ LANGUAGE plpgsql;
