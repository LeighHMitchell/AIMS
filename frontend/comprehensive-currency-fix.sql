-- Comprehensive Currency Conversion Fix Script
...
SELECT 'Reset ' || ROW_COUNT() || ' failed transactions for retry' as result;
...
BEGIN
    ...
    LOOP
        ...
        SELECT exchange_rate INTO rate_record
        FROM exchange_rates 
        ...
        IF FOUND THEN
            ...
            usd_amount := ROUND(txn_record.value * rate_record.exchange_rate, 2);
            
            -- Update transaction
            UPDATE transactions 
            SET 
                value_usd = usd_amount,
                exchange_rate_used = rate_record.exchange_rate,
            ...
            exchange_rate := rate_record.exchange_rate;
            status := 'CONVERTED';
            RETURN NEXT;
        ELSE
            -- Mark as failed
