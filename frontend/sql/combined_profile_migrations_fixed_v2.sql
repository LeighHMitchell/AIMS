-- Combined Profile & Notifications Migration v2 (Fixed column references)
...
BEGIN
  ...
  SET 
    ...
    updated_at = NOW()
  WHERE user_id = NEW.id;  -- Changed from NEW.auth_id to NEW.id
...
BEGIN
  ...
  SELECT * INTO user_record FROM users WHERE id = NEW.user_id;  -- Changed from auth_id to id
  ...
  IF user_record IS NOT NULL THEN
    ...
    SET 
      ...
      updated_at = NOW()
    WHERE id = NEW.user_id;  -- Changed from auth_id to id
  END IF;
...
CREATE POLICY "Users can view their own record" ON users
  FOR SELECT USING (auth.uid() = id);  -- Changed from auth_id to id

CREATE POLICY "Users can update their own record" ON users
  FOR UPDATE USING (auth.uid() = id);  -- Changed from auth_id to id
